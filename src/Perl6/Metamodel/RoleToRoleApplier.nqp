my class RoleToRoleApplier {
    method apply($target, @roles) {
        ...
        for %meth_info {
            ...
            # that does not have the "face" trait.
            if nqp::existskey(%target_meth_info, $name) {
                ...
                for @add_meths {
                    ...
                    unless $face {
                        if nqp::iseq_s($provider.HOW.name($provider),'Perl6::Metamodel::CurriedRoleHOW') {
                            $face := $provider.is_face;
                        }
                        else {
                            $face := $provider.HOW.is_face($provider);
                        }
                    }
                    unless $face {
                        unless $non_faces {
                            if nqp::iseq_s($provider.HOW.name($provider),'Perl6::Metamodel::CurriedRoleHOW') {
                                $first_non_face := $provider.name($provider);
                            }
                            else {
                                $first_non_face := $provider.HOW.name($provider)
                            }
                        }
                        $non_faces++
                    };
                ...
                if $non_faces > 0 {
                    my $mess := 'Warning: Overrode method "'
                       ~ $name ~ '" from role "' ~ $first_non_face
                       ~ '" -- Mark that method (or that role) as "is face"'
                       ~ ' to improve codebase maintainabilty.';
                    note($mess);
                }
            }
            else {
                ...
                if +@add_meths == 1 {
                    ...
                    # compositions.
                    my @impl_meths;
                    my $diamond := False; # Which is !=:= NQPMu
                    for @add_meths {
                        my $yada := 0;
                        my $pack;
                        try { $yada := $_.yada; }
        ...
        if nqp::can($target.HOW, 'private_method_table') {
            ...
            for %priv_meth_info {
                ...
                if nqp::existskey(%target_priv_meth_info, $name) {
                    ...
                    if $non_faces > 0 {
                        my $mess := 'Warning: Overrode private method "!'
                           ~ $name ~ '" from role "' ~ $first_non_face
                           ~ '" -- Mark that method (or that role) as "is face"'
                           ~ ' to improve codebase maintainabilty.';
                        note($mess);
        ...
        if nqp::can($target.HOW, 'multi_methods_to_incorporate') {
            ...
            for %multi_methods_to_incorporate {
                ...
                # Do we already have a method of this name/sig? If so, ignore all of the
                # methods we have from elsewhere, but complain if we ignore one
                # that does not have the "face" trait.
                if nqp::existskey(%multis_by_name, $name) {
                        ...
                        if Perl6::Metamodel::Configuration.compare_multi_sigs($code, $to_add) {
                            ...
                            unless $face {
                                unless $non_faces {
                                    $nope_code := $to_add;
                                    if nqp::iseq_s($provider.HOW.name($provider),'Perl6::Metamodel::CurriedRoleHOW') {
                                        $nope_provider := $provider.name($provider);
                                    }
                                    else {
                                        $nope_provider := $provider.HOW.name($provider);
                                    }
                                }
                                $non_faces++;
                            };
                    ...
                    if $non_faces > 0 {
                        my $sigstr := "";
                        my $sig := $nope_code.signature;
                        if nqp::isconcrete($sig) && nqp::can($sig, 'gist') {
                            $sigstr := $sig.gist;
                        }
                        my $mess := 'Warning: Overrode multimethod candidate "'
                           ~ $name ~ $sigstr ~ '" from role "' ~ $nope_provider
                           ~ '" -- Mark that candidate (or that proto, or that role) as "is face"'
                           ~ ' to improve codebase maintainabilty.';
                        note($mess);
                    }
                }
        ...
        for @roles {
            ...
            for @attributes {
                ...
                my $add_pack := False; # Which is !=:= NQPMu
